/**
 * AlphaRSI — 三角色自改进闭环 skill 的 DSH 打包入口。
 *
 * 双轨单源：本包同时承载 Claude Code 插件结构（.claude-plugin/ + plugins/）
 * 与 DSH/npm 结构（package.json + src/ + cordis.patch.yml）。SKILL.md 只维护一份，
 * 住在 plugins/alpharsi/skills/ 下，这里直接把 SKILLS_ROOT 指过去，不做任何复制。
 *
 * AlphaRSI 是 workflow skill（驱动固定 Python 闭环，零代码生成），
...
// ---------------------------------------------------------------------------
export const name = 'alpharsi';
export const inject = ['skills'];

const PACKAGE_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), '..');
// 双轨单源：SKILL.md 的唯一真身在 plugins/alpharsi/skills/，Claude Code 侧也读这里。
const SKILLS_ROOT = join(PACKAGE_ROOT, 'plugins', 'alpharsi', 'skills');
...
class AlpharsiSkillProvider implements SkillProvider {
...
export function apply(ctx: Context) {
  ctx.skills.registerProvider(
    () => new AlpharsiSkillProvider('alpharsi', (message) => ctx.logger?.warn(message)),
  );
}
